...
namespace WAVCombine {
    ...
    CheckResult preCheck(QString rootDirName, QString dstWAVFileName, bool recursive, AudioIO::WavAudioFormat targetFormat){
        ...
        auto formats = QtConcurrent::mapped(wavFileNames, std::function([](const QString& fileName)->QPair<QString, AudioIO::WavAudioFormat>{
            try {
        ...
        auto hasUnknownType = QtConcurrent::filtered(formats, [](const QPair<QString, AudioIO::WavAudioFormat>& info) -> bool{
        ...
        auto hasMultipleChannels = QtConcurrent::filtered(formats, [targetFormat](const QPair<QString, AudioIO::WavAudioFormat>& info) -> bool{
        ...
        auto hasLargerSampleRate = QtConcurrent::filtered(formats, [targetFormat](const QPair<QString, AudioIO::WavAudioFormat>& info) -> bool{
        ...
        for (const auto& i : std::as_const(hasLargerSampleRate)){
            warningMsg.append(QCoreApplication::translate("WAVCombine", "<p class='warning'>Sample rate (%2 Hz) of \"%1\" is larger than target (\"%3\" Hz)."
                                                                        "The precision could be lost a bit when processing.</p>").arg(i.first).arg(i.second.kfr_format.samplerate).arg(targetFormat.kfr_format.samplerate));
        }

        auto hasLargerQuantization = QtConcurrent::filtered(formats, [targetFormat](const QPair<QString, AudioIO::WavAudioFormat>& info) -> bool{
           return (kfr::audio_sample_type_to_precision(info.second.kfr_format.type) > kfr::audio_sample_type_to_precision(targetFormat.kfr_format.type)) || (kfr::audio_sample_is_float(info.second.kfr_format.type) && (!kfr::audio_sample_is_float(targetFormat.kfr_format.type)));
        }).results();

        for (const auto& i : std::as_const(hasLargerQuantization)){
             warningMsg.append(QCoreApplication::translate("WAVCombine", "<p class='warning'>Bit depth (%2) of \"%1\" is larger than target."
                                                                        "The precision could be lost a bit when processing.</p>").arg(i.first).arg(kfr::audio_sample_type_to_string(i.second.kfr_format.type).data()));
        }

        if (warningMsg.isEmpty()) return {CheckPassType::OK, "", wavFileNames};
    ...
    {
        return QtConcurrent::mappedReduced(WAVFileNames,
        //Map
        std::function([rootDirName, targetFormat](const QString& fileName) -> QPair<std::shared_ptr<kfr::univector2d<wavtar_defines::sample_process_t>>, QJsonObject>{
            ...
            if (std::abs(inputSampleRate - outputSampleRate) > 1e-5) {
                 ...
                 for (size_t c = 0; c < outputChannels; ++c) {
                    if (c >= processedData.size()) continue;
                    
                     auto converter = kfr::sample_rate_converter<T>(kfr::sample_rate_conversion_quality::normal, (size_t)outputSampleRate, (size_t)inputSampleRate);
            ...
            switch(result.format.container){
                case AudioIO::WavAudioFormat::Container::RIFF: originalWavFormat = 0; break; // kfr::audio_format::riff
                case AudioIO::WavAudioFormat::Container::RF64: originalWavFormat = 2; break; // kfr::audio_format::rf64
                case AudioIO::WavAudioFormat::Container::W64:  originalWavFormat = 1; break; // kfr::audio_format::w64
                default: break;
            }
            ...
            auto retData = std::make_shared<kfr::univector2d<wavtar_defines::sample_process_t>>(std::move(processedData));
        ...
        //Reduce
        std::function([targetFormat](QPair<std::shared_ptr<kfr::univector2d<wavtar_defines::sample_process_t>>, QJsonObject>& total,
                       const QPair<std::shared_ptr<kfr::univector2d<wavtar_defines::sample_process_t>>, QJsonObject>& input){
            
            // Initialization
            if (total.first == nullptr) {
                total.first = std::make_shared<kfr::univector2d<wavtar_defines::sample_process_t>>(targetFormat.kfr_format.channels);
            ...
            for(size_t c = 0; c < nChannels; ++c) {
                // Resize and copy
                 ...
                 std::copy(input.first->operator[](c).begin(), input.first->operator[](c).end(), total.first->operator[](c).begin() + currentSize);
            }
            ...
            total.second.insert("total_duration", samplesToTimecode(currentSize + appendSize, targetFormat.kfr_format.samplerate));
        })
        );
    }
