...
TranslationManager::TranslationManager()
{
    ...
    // Note: This will include the source language (en) which is auto-generated by Qt
    // for plural form handling, not a full translation. See qt_add_translations() docs.
    QDir i18nDir(":/i18n");
    QStringList qmFiles = i18nDir.entryList(QStringList() << "KiraWavTar_*.qm", QDir::Files);

    for (const QString &fileName : qmFiles) {
        // Extract locale string from filename (e.g., "KiraWavTar_zh_CN.qm" -> "zh_CN")
        QString localeName = fileName;
        localeName.remove("KiraWavTar_").remove(".qm");
